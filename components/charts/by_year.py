...
def by_year(advanced_report=False):
    ...
    if not st.session_state.selected_years:
        ...
        with row2[0]:
            ...
            st.caption(f"Recurso empenhado pela Direção do IFC - Araquari em {', '.join(map(str, st.session_state.selected_years))}")
        ...
        with row2[1]:
            ...
            st.caption(f"Empenho liquidado pela Direção do IFC - Araquari em {', '.join(map(str, st.session_state.selected_years))}")
